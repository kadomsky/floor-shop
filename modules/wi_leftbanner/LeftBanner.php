...
class LeftBanner extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'wileftbanner_slides',
		'primary' => 'id_wileftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'wileftbanner` (`id_shop`, `id_wileftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'wileftbanner`
			WHERE `id_wileftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_wileftbanner_slides` = hs.`id_wileftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_wileftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'wileftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'wileftbanner` hs ON (hss.`id_wileftbanner_slides` = hs.`id_wileftbanner_slides`)
