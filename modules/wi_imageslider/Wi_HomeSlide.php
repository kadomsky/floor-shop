...
class Wi_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'wihomeslider_slides',
		'primary' => 'id_wihomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'wihomeslider` (`id_shop`, `id_wihomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'wihomeslider`
			WHERE `id_wihomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_wihomeslider_slides` = hs.`id_wihomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_wihomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'wihomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'wihomeslider` hs ON (hss.`id_wihomeslider_slides` = hs.`id_wihomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Wi_HomeSlide($row['id_slide']);
