...
include_once('wi_imageslider.php');

$home_slider = new Wi_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'wihomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_wihomeslider_slides` = '.(int)$id_slide
		);
